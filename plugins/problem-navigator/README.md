...
This is the single release bundle, `problem-navigator-2.1.0.zip`. Keep `skills/`, `mcp/`, and `release.json` under the same installation root. The shared core appears once; optional Codex metadata and its guide are included in the same bundle. It is not a universal one-click plugin import format.

## Quick start

Use an AI host that can read complete skill files and references, read/write local artifacts, run local Python commands, and ask for clarification and document acceptance. Prepare Python 3.11+ and `uv` on that host's `PATH`. From this bundle's root:
...
The same bundle supports manual core loading and the optional Codex adapter. Its Codex guide is `adapters/codex/README.md`; the archive-root `.agents/plugins/marketplace.json` points to the archive root, and `.codex-plugin/plugin.json` contains the inline MCP registration. No separate Codex download is needed.
